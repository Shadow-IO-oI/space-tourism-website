(() => {
    ...
    function initSliders() {
        if (document.querySelector(".crew__slider")) new core(".crew__slider", {
    ...
    const sections = document.querySelectorAll('section[class^="page__"]');
    links.forEach((link => {
        link.addEventListener("click", (event => {
            event.preventDefault();
            links.forEach((l => l.classList.remove("active")));
            link.classList.add("active");
            const pageClass = link.dataset.page;
            sections.forEach((section => {
                if (section.classList.contains(pageClass)) section.classList.add("active"); else section.classList.remove("active");
            }));
        }));
    }));
    const destinationMenuLinks = document.querySelectorAll(".destination-menu__link");
    destinationMenuLinks.forEach((link => {
        link.addEventListener("click", (event => {
            event.preventDefault();
            destinationMenuLinks.forEach((l => l.classList.remove("active")));
            link.classList.add("active");
        }));
    }));
    function updateDestinationContent(event) {
        event.preventDefault();
        ...
        fetch("./json/data.json").then((response => response.json())).then((data => {
            ...
            const destination = data.destinations.find((dest => dest.name.toLowerCase() === page.toLowerCase()));
            console.log("Destination:", destination);
            const destinationIMG = document.querySelector(".destination__image");
            destinationIMG.classList.remove("show");
            setTimeout((() => {
                document.querySelector(".destination__image img").src = destination.images.png;
                destinationIMG.classList.add("show");
            }), 300);
            ...
            const destinationDistance = document.querySelector(".info-destination__distance .info-destination__value");
            destinationDistance.classList.remove("show");
            setTimeout((() => {
                destinationDistance.textContent = destination.distance;
                destinationDistance.classList.add("show");
            }), 300);
            const destinationTravel = document.querySelector(".info-destination__travel .info-destination__value");
            destinationTravel.classList.remove("show");
            setTimeout((() => {
                destinationTravel.textContent = destination.travel;
                destinationTravel.classList.add("show");
            }), 300);
            const destinationDescription = document.querySelector(".destination__description");
            destinationDescription.classList.remove("show");
            setTimeout((() => {
                destinationDescription.textContent = destination.description;
                destinationDescription.classList.add("show");
            }), 300);
        })).catch((error => console.error(error)));
    }
    destinationMenuLinks.forEach((link => link.addEventListener("click", updateDestinationContent)));
    window["FLS"] = true;
    isWebp();
    menuInit();
})();